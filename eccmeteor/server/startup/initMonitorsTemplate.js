...
initSvseMonitorsTemplateAtStartUp = function(debug){
	...
	if(!result){
		Log4js.info("initSvseMonitorsTemplateAtStartUp failed,svGetAllMonitorTempletInfo exists errors!!",-1);
		return;
	}
	...
	for(id in result["monitors"]){
		...
		if(!monitor){
			Log4js.info("initSvseMonitorsTemplateAtStartUp  failed,svGetMonitorTemplet " +id+" exists errors!!",-1);
			continue;
		}
		SvseMonitorTemplate.insert(monitor,function(err,r_id){
			if(err)Log4js.info(err,-1);
		});
	}
	Log4js.info("模板更新完成！");
}