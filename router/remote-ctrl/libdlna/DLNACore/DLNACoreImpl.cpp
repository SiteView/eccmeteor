...
#ifdef _REMOTE_DLNA_
NPT_Result DLNACoreImpl::browseMediaServerEx(const NPT_String& requestFrom, const UUID& mediaServerUuid, const NPT_String& containerId, NPT_UInt32 step, DLNAProgressiveBrowseOp::ResultCallback *resultCallback, DLNAProgressiveBrowseOp **op)
{
	*op = NULL;
	...
	DLNAFullBrowseOpImpl *impl = new DLNAFullBrowseOpImpl(resultCallback);
	impl->addRef();
	DLNAFullBrowseTask *task = new DLNAFullBrowseTask(requestFrom, impl, m_cp, mediaServerUuid, dmsInfo->m_cds->serviceId(), containerId, step);
	NPT_Result nr = m_cp->startTask(task);
	if (NPT_FAILED(nr)) {
		impl->release();
		return nr;
