...
#ifdef _REMOTE_DLNA_
void DLNAFullBrowseOpImpl::notifyBrowseResult(const NPT_String& requestFrom, NPT_UInt32 startingIndex, NPT_UInt32 numberReturned, NPT_UInt32 totalMatches, const DLNAObjectList& ls)
{
	ReadLocker locker(m_stateLock);
	if (m_resultCallback) {
		m_resultCallback->onDLNAProgressiveBrowseOpResult(this, startingIndex, numberReturned, totalMatches, ls, requestFrom);
	}
}
#endif
...
#ifdef _REMOTE_DLNA_
DLNAFullBrowseTask::DLNAFullBrowseTask(const NPT_String& requestFrom, DLNAFullBrowseOpImpl *op, ControlPoint *cp, const UUID& mediaServerUuid, const NPT_String& cdsId, const NPT_String& containerId, NPT_UInt32 requestedCount)
	: m_requestFrom(requestFrom), m_op(op), m_cp(cp), m_mediaServerUuid(mediaServerUuid), m_cdsId(cdsId), m_containerId(containerId), m_requestedCount(requestedCount)
	, m_actInst(NULL)
{
	m_op->addRef();
	m_op->setBuddy(this);
...
void DLNAFullBrowseTask::exec()
{
...
#ifdef _REMOTE_DLNA_
	NPT_Int32 type = 0;
	NPT_List<NPT_String> tmp_list;
	if (m_requestFrom.GetLength() > 0)
	{
		//requestFrom for MES_REQ_OBJ_LISTS : mes_type&&xmpp_id&&jid&&startIndex&&requestCnt
		tmp_list = m_requestFrom.Split("&&");
		(*tmp_list.GetItem(0)).ToInteger32(type);	//mes_type
	}

	//请求文件列表的方式为分批请求，终端发送需要的列表index区间，避免文件列表太大造成大延时
	if (type == 2)	//MES_REQ_OBJ_LISTS			define in remotedlna.h
	{
		DLNAObjectList ls1;
		NPT_UInt32 totalMatches;
		
		(*tmp_list.GetItem(3)).ToInteger32(startingIndex);	//startIndex
		(*tmp_list.GetItem(4)).ToInteger32(requestedCount);	//requestCnt
		...
		if (NPT_SUCCEEDED(nr)) {
			...
			{
				m_op->notifyBrowseResult(m_requestFrom, startingIndex, numberReturned, totalMatches, ls1);
	...
	else	//请求其他列表内容(如设备列表等)时，保留库原有的机制，一次性返回结果
	{
#endif
	for (;;) {
		DLNAObjectList ls1;
	...
	if (!firstNotify) {
#ifdef _REMOTE_DLNA_
		m_op->notifyBrowseResult(m_requestFrom, 0, 0, 0, ls);
#else
