...
#ifdef _REMOTE_DLNA_
NPT_Result DLNACore::browseMediaServerEx(const NPT_String& requestFrom, const UUID& mediaServerUuid, const NPT_String& containerId, int step, DLNAProgressiveBrowseOp::ResultCallback *resultCallback, DLNAProgressiveBrowseOp **op)
{
	return m_impl->browseMediaServerEx(requestFrom, mediaServerUuid, containerId, step >= 0 ? step : 30, resultCallback, op);
}
