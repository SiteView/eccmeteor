...
#ifdef	WIN32
#include "windows.h"
//cxy 2013/06/14 share 
#define KEY_PATH		"SOFTWARE\\SiteView\\SiteViewECC"
#define KEY_PATH_CORE	"SOFTWARE\\SiteView\\SiteViewCore"
#define KEY_PATH_NNM	"SOFTWARE\\SiteView\\SiteViewCore"
//#define KEY_PATH_NNM	"SOFTWARE\\SiteView\\SiteViewNNM"
#define KEY_PATH_DM	    "SOFTWARE\\SiteView\\SiteViewDM"

//#define KEY_NAME		"root_path_7"
#define KEY_NAME	"Path"
...
void outputRegeditKey(string g_dbPName)
{
	//cxy 2013/06/14 share 
	string strtmp = KEY_PATH;
	
	if(g_dbPName.find("svdb.exe") != -1)
	{
	}
	else if(g_dbPName.find("svdbnnm.exe") != -1)
	{
		strtmp = KEY_PATH_NNM;
	}	
	else if(g_dbPName.find("svdbdm.exe") != -1)
	{
		strtmp = KEY_PATH_DM;
	}
	else
	{
		strtmp = KEY_PATH_CORE;
	}
	//}
	//if(strcmp(g_dbPName.c_str(), "svdb.exe") == 0 || strcmp(g_dbPName.c_str(), "svdb") == 0)
	//{
	//	strtmp = KEY_PATH;
	//}
	//else if(strcmp(g_dbPName.c_str(), "svdbnnm.exe") == 0 || strcmp(g_dbPName.c_str(), "svdbnnm") == 0)
	//{
	//	strtmp = KEY_PATH_NNM;
	//}	
	//else if(strcmp(g_dbPName.c_str(), "svdbdm.exe") == 0 || strcmp(g_dbPName.c_str(), "svdbdm") == 0)
	//{
	//	strtmp = KEY_PATH_DM;
	//}
	//else
	...
	string path= GetRootPathByKey(strtmp.c_str());

	cout<<"    regedit key for svdb reg test : "<<strtmp.c_str()<<"\\"<<KEY_NAME<<endl;

	if( !path.empty() )
		//cout<<"    regedit key for svdb : "<<KEY_PATH_CORE<<"\\"<<KEY_NAME<<endl;
		cout<<"    regedit key for svdb : "<<strtmp.c_str()<<"\\"<<KEY_NAME<<endl;
	else
	{
...
std::string GetRootPathFromKeyCore(string g_dbPName)
{
	//cxy 2013/06/14 share 
	string strtmp = KEY_PATH;
	if(g_dbPName.find("svdb.exe") != -1)
	...
	//string path= GetRootPathByKey(KEY_PATH_CORE);
	string path= GetRootPathByKey(strtmp);
