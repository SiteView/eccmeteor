...
namespace SV
{
	...
	SV_API bool GetSVDYN(string monitorid,SVDYN &dyn,string user,string addr)
	{
	...
	SV_API bool GetSVDYNNODisplayString(string monitorid,SVDYN &dyn,string user,string addr)
	{
		if(monitorid.empty()||user.empty()||addr.empty())
			return false;
	...
	SV_API bool SetDYN(string monitorid,int type,string user,string addr)
	{
		if(monitorid.empty())
			return false;
	...
	/////////////////////////////////DB///////////////////////////
	SV_API RECORDSET QueryRecords(string monitorid,TTimeSpan recenttimespan,string user,string addr)
	{
		if(monitorid.empty()||user.empty()||addr.empty())
	...
	SV_API bool DeleteRecords(string monitorid,svutil::TTime before,string user,string addr)
	{
		if(monitorid.empty()||user.empty()||addr.empty())
			return false;
		SVDBQUERY query={0};
	...
	SV_API RECORDSET QueryRecords(string monitorid,int count,string user,string addr)
	{
		if(monitorid.empty()||user.empty()||addr.empty())
			return INVALID_VALUE;
	...
	SV_API RECORDSET QueryRecords(string monitorid,TTime begin,TTime end,string user,string addr)
	{
		if(monitorid.empty()||user.empty()||addr.empty())
			return INVALID_VALUE;
	...
	SV_API bool QueryRecordCount(string monitorid,int & count,string user,string addr)
	{
		if(monitorid.empty()||user.empty()||addr.empty())
			return false;
	...
	SV_API bool GetAllTableNames(std::list<string> &namelist,string user,string addr)
	{
	...
	SV_API bool InsertTable(string monitorid,int monitortype,string user,string addr)
	{
		if(monitorid.empty()||user.empty()||addr.empty())
			return false;
	...
	SV_API bool DeleteTable(string monitorid,string user,string addr)
	{
		if(monitorid.empty()||user.empty()||addr.empty())
			return false;
	...
	////////////////////////////
	SV_API bool AppendRecord(string monitorid,const char *data,unsigned int len,string user,string addr)
	{
		if(monitorid.empty()||user.empty()||addr.empty())
			return false;
	...
	SV_API bool AppendMassRecord(std::list<SingelRecord> & listrcd,string user,string addr)
	{
	...
	////////////////////////////
	SV_API bool SetLogKeepDays(int days,string user,string addr)
	{
	...
	SV_API bool GetLogKeepDays(int &days,string user,string addr)
	{
		string rootpath=GetRootPath();
		if(rootpath.empty())
		{
