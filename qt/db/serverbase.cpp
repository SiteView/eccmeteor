...
bool CConfigFile::SaveData(string & estr)
{
	...
	try{
		if(!CheckFileSize(mf_configdbFile.c_str(),len))
		{
			printf("failed to CheckFileSize\n");
			return false;
		}
...
bool CConfigFile::LoadData(string & estr)
{
	...
	try{
		...
		ost::MappedFile file(mf_configdbFile.c_str(),0,len,ost::File::accessReadWrite);
		char*p=(char *)file.fetch(0,len);
		if(p)
		{
...
bool CWholeConfig::SaveData(string & estr)
{
	...
	try{
		if(!CheckFileSize(mf_configdbIndexFile.c_str(),len))
		{
			printf("failed to CheckFileSize\n");
			return false;
		}
...
bool CWholeConfig::LoadData(string & estr)
{
	...
	try{
		...
		ost::MappedFile file(mf_configdbIndexFile.c_str(),0,len,ost::File::accessReadWrite);
		char*p=(char *)file.fetch(0,len);
		if(p)
		{
