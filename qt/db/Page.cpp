...
#else
	if(-1 == lseek(m_ppt->m_hd,pos,SEEK_SET))
	{
		printf("failed to move file pointor\n");
		return false;
	}
	int io = ::read(m_ppt->m_hd,m_rawdata,m_ppt->GetPagePoolHead().m_PageSize);

	if((size_t) io != m_ppt->GetPagePoolHead().m_PageSize)
	{
		printf("swapin failed. read file failed.\n");
		return false;
	}
#endif
...
bool Page::swapout(bool isdel)
{
	...
	if(m_ischange)
	{
		...
		int io = ::write(m_ppt->m_hd,m_rawdata,m_ppt->GetPagePoolHead().m_PageSize);
...
bool Page::swapinex()
{
	...
	memset(m_rawdata,0,tPageSize);

	long pos= tPageSize;
    pos=pos*m_mpos.m_pos+sizeof(struct PagePool::PagePoolHead);

#ifdef WIN32
	if(!::SetFilePointer(tHd,pos,NULL,FILE_BEGIN))
	{
		printf("swapinex failed move file pointor error errorid :%d\n",::GetLastError());
		return false;
	}

	DWORD size=0;
	if(!::ReadFile(tHd,m_rawdata,tPageSize,&size,NULL))
	{
		printf("swapinex failed, read file failed:%d\n",::GetLastError());
		return false;
	}
#else
	if(-1 == lseek(tHd,pos,SEEK_SET))
	{
		printf("failed to move file pointor\n");
		return false;
	}
	int io = ::read(tHd,m_rawdata,tPageSize);

	if((size_t) io != tPageSize)
	{
		printf("swapinex failed, read file failed. pos:%d, io:%d, tPageSize:%d, tHd:%d, fileindex:%d\n",pos,io,tPageSize,tHd,m_mpos.m_fileindex);
		return false;
	}
#endif
...
bool Page::swapoutex(bool isdel)
{
	...
	if(m_ischange)
	{
		...
		{
			puts("swapex out -- Set file pointer failed");
			return false;
		}
		DWORD size=0;
		if(m_rawdata!=NULL)
		{
			...
			{
				printf("swapex(write) failed errorid:%d\n",::GetLastError());
				return false;
			}
		}
#else
		if(-1 == lseek(m_ppt->m_fhd[m_mpos.m_fileindex]->m_hd,pos,SEEK_SET))
		{
			printf("failed to move file pointor\n");
			return false;
		}
		int io = ::write(m_ppt->m_fhd[m_mpos.m_fileindex]->m_hd,m_rawdata,m_ppt->GetPagePoolHead().m_PageSize);

		if((size_t) io != m_ppt->GetPagePoolHead().m_PageSize)
		{
			printf("swapex(write) failed.\n");
			return false;
		}
#endif
