...
RecordValue::RecordValue(const RecordValue &value)
{
	...
	case	DataType::stringtype:
		if(value.dvalue.sv)
			dvalue.sv=S_STRDUP(value.dvalue.sv);
		break;
	default: 
		dvalue.sv=NULL; 
		break;
...
{
	vt=DataType::stringtype;
	dvalue.sv=S_STRDUP(value.c_str());
	return *this;
}
...
bool RecordSet::CreateObjectByRawData(StringMap *resMap,const char *buf,S_UINT len)
{
	...
	try{
		...
		for(int n=0;n<RecordCount;n++)
		{
			...
			if(badRCD)
			{
				prd->m_error= S_STRDUP(badStr.c_str());
			...
			{
				strcat(dstr,".");
				prd->m_displaystr=S_STRDUP(dstr);
			}
			if(prd!=NULL)
				m_records.push_back(prd);
		}
