...
bool CSnmpOperate::RetrieveData(CSVSnmpOID objOID, int i, CSVExpression& expression)
{
	...
	unsigned long lTime = 5;  //2秒太短了，取出的数据给上次是一样的，故出现0。  duanYi 2010-06-10
	mysleep(lTime * 1000);
...
bool CSnmpOperate::GetResult(char* chReturn, int & nSize)
{
	...
	{ //初始化SNMP
		...
		{ //得到每一个oid的结果
			...
			if ((strstr(chOID, "1.3.6.1.2.1.2.2.1.12") != NULL) || (strstr(chOID, "1.3.6.1.2.1.2.2.1.18") != NULL)) // 针对特殊的防火墙的接口
			{
		...
		if (bReturn)
		{
			try
			{
				...
				{			//根据每一个公式计算结果
					...
					{ //如果计算公式中存在错误或者计算中出现错误（如：除数为0） //duanYi 只要有一个返回值错了，应该报计算错误的警告,
