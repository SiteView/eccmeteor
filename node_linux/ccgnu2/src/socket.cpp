...
bool DCCPSocket::setCCID(uint8 ccid)
{
	...
	if (ret < 0) {
		cout << "Can not determine available CCIDs" << endl;
		return false;
	}
	...
	if (!ccid_supported) {
		cout << "CCID specified is not supported" << endl;
		return false;
	}

	if (setsockopt(so, SOL_DCCP, DCCP_SOCKOPT_CCID, (char *)&ccid, sizeof (ccid)) < 0) {
		cout << "Can not set CCID" << endl;
		return false;
	}
...
int DCCPSocket::getTxCCID()
{
    ...
    if (ret < 0) {
        cout << "Can not determine get current TX CCID value" << endl;
        return -1;
    }
    return ccid;
}
...
int DCCPSocket::getRxCCID()
{
    ...
    if (ret < 0) {
        cout << "Can not determine get current RX CCID value" << endl;
        return -1;
    }
    return ccid;
}
