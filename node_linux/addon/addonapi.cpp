...
SCA_SVAPI
bool sca_svapi::ScaGetUnivData(ForestMap & fmap,const NodeData & inwhat, string & estr)
{
	fmap.clear();
...
SCA_SVAPI
bool sca_svapi::ScaSubmitUnivData(ForestMap & fmap, const NodeData & inwhat, string & estr)
{
	string what=GetValueInNodeData(inwhat,"dowhat",estr);
	if(what.empty())
		return false;
...
SCA_SVAPI
bool sca_svapi::ScaGetForestData(ForestList & flist, const NodeData & inwhat, string & estr)
{
	string what=GetValueInNodeData(inwhat,"dowhat",estr);
	if(what.empty())
		return false;
	...
	if(what.compare("GetTreeData")==0)
	{
		string parentid=	GetValueInNodeData(inwhat,	"parentid",estr);
		string str;
		string LoginName=	GetValueInNodeData(inwhat,	"LoginName",str);
		string onlySonStr=	GetValueInNodeData(inwhat,	"onlySon",str);
