...
void CSVExpression::AddFields(const string &pcszRow, const string &cszField, UInt64 ulValue)
{
...
int CSVExpression::calculateDouble(const string &szExpr, double &dResult)
{
    ...
    if((nRet = toRPN(szExpr, szRpn)) == eval_ok) // 根据name，解析算法 8 oid7 * oid7.time 1024 * /
    {
		...
		printf("CSVExpression::calculateDouble(Expr,Rpn,Result):%s, %s, %.2f\n",szExpr.c_str(),szRpn.c_str(),dResult);
    }
    return nRet;
}
...
UInt64 CSVExpression::GetOidValue(const string &chOID)
{
    UInt64 ulValue = 0;
