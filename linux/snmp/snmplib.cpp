...
extern void myPrintLog(const char *LogMes);
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// start CSVBaseSNMP
...
#else
        int nlen = sprintf(szMsg, "Error in --*GetBulkRequest*---");
#endif
...
#else
        int nlen = sprintf(szMsg, "Error in --*GetRequest*---");
#endif
        ...
        if(m_pSnmp)
        {
            ...
            for (int i=0; i<pdu.get_vb_count(); i++)
            {
                ...
                if ( vb.get_syntax() != sNMP_SYNTAX_ENDOFMIBVIEW)
                {
                    ...
                    cout << "CSVBaseSNMP::GetRequest, result.m_szValue = "<< result.m_szValue.c_str() << endl;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
UInt64 CSVSnmpOID::AvgValue(const MonitorResult &ResultList)
{
    if(!m_bSum)
    {
        m_szIndexCount = 0;
        ...
        for(resultConstItem it = ResultList.begin(); it != ResultList.end(); it ++)
        {
            if(!m_szSelName.empty() && it->second.m_szValue == m_szSelName)
            {
                uValue = strToUI64(it->second.m_szValue.c_str(), error_no);
                m_szSumValue += uValue;
                if(m_ulMax < uValue)
                    m_ulMax = uValue;
                if(m_ulMin > uValue)
                    m_ulMin = uValue;
                m_avgValue += static_cast<UInt64>(uValue/m_szIndexCount);
            }
            else if(m_szSelName.empty())
            {
                uValue = strToUI64(it->second.m_szValue.c_str(), error_no);
...
//    if(m_szIndexCount > 0)
//        return static_cast<UInt64>(m_szSumValue / m_szIndexCount);
//    else
//        return 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
UInt64 CSVSnmpOID::MaxValue(const MonitorResult &ResultList)
{
    if(!m_bSum)
    	AvgValue(ResultList);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
UInt64 CSVSnmpOID::MinValue(const MonitorResult &ResultList)
{
    if(!m_bSum)
    	AvgValue(ResultList);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
UInt64 CSVSnmpOID::SumValue(const MonitorResult &ResultList)
{
    if(!m_bSum)
    	AvgValue(ResultList);
...
int CSVSnmpOID::GetResult(MonitorResult &ResultList)
{
    ...
    {//如果OID类型是简单变量
        ...
        printf("CSVSnmpOID::GetResult, get simple request.\n");
    }
    else
    {//如果是表格变量
        ...
        {//如果索引值为空，取表格变量
			...
			printf("CSVSnmpOID::GetResult, If oidindex is empty, get bulk request.\n");
        }
        else
        {//索引值不为空，取简单变量
			...
			printf("CSVSnmpOID::GetResult, If oidindex isn't empty, get simple request.  chOIDTmp = %s chOIDTmpLen = %d\n", chOIDTmp, strlen(chOIDTmp));
...
std::string TrimForUI64(const std::string & input)
{
	string ret = input;
	try
	{
...
UInt64 strToUI64(const char * c0, int & error)
{
	try
	{
		string s0 = TrimForUI64(c0);
		if(s0.empty())
			return 0;
		const char * c1 = s0.c_str();
//		printf("c1:%s\n", c1);
		UInt64 v = _atoi64(c1);
		char c2[32] = { 0 };
#ifdef WIN32
		...
		if (size >= 21)
		{
			printf("  Error(1), input too big in strToUI64:%s\n", c0);
			error = 1;
			return 0;
		}
		if (size != 19 && size != 20)
		{
			printf("  Error(2) in strToUI64( %s )\n", c0);
			error = 2;
			return 0;
		}
		int n_last = (size == 19) ? 1 : 2;

		string s_last = s1.substr(size - n_last, n_last);
		int v_last = atoi(s_last.c_str());
		string s_init = s1.substr(0, size - n_last);
		UInt64 v_init = _atoi64(s_init.c_str());
...
//		printf("c3:%s\n", c3);
		...
		if(s1.compare("18446744073709551616")>=0)
		{
			printf("  Error(3), input bigger than 2^64 in strToUI64:%s\n", c0);
			error = 3;
			return 0;
		}

		printf("  Error(4) in strToUI64( %s )\n", c0);
		error = 4;
		return 0;

	} catch (...)
	{
		printf("  Exception(5) in strToUI64( %s )\n", c0);
		error = 5;
		return 0;
	}
}
