...
bool SvdbMain::Init(void)
{
	...
	if (IdcUser::EnableConfigDB)
	{
		...
		m_pWholeConfig = new CWholeConfig(IdcUser::RootPath + "/data/configdbIndex.data", cdbpath + "/",
				historypath + "/");
		string estr;
	...
	if (IdcUser::AutoResolveIDS)
	{
		...
		if (m_pLanguage == NULL)
		{
			...
			cout << " ********************************************************************************\n" << endl;
			cout << "    Fatal error of Template file: " << buf << endl;
			cout << "\n ********************************************************************************" << endl;
		}
	...
	int clk_tck= CLK_TCK;
#else
	int clk_tck = CLOCKS_PER_SEC;
#endif
	printf("Start all %d DB heads:  %.2f seconds\n\n", IdcUser::Users.size() + 1,
			(double) (time2 - time1 + time4) / clk_tck);
	...
	string text = "\n\n(" + IdcUser::ProcessID + ")svdb starts working... \n";
	printf(text.c_str());
	AddToErrorLog("SVDB server starts working...");
	return true;
