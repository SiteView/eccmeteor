...
bool ResourcePool::Load(void)
{
	...
	{
		printf("Load ResourcePool Exception, %s\n", m_FilePath.getword());
		return false;
	}
...
bool ResourcePool::Submit()
{
	...
	try{
		...
		{
			printf("failed to CheckFileSize,in ResourcePool::Submit\n");
			return false;
		}
...
BOOL ResourcePool::CreateObjectByRawData(const char *lpbuf,S_UINT bufsize)
{
	...
	{
		puts("Exception happended in ResourcePool::CreateObjectByRawData.");
		return false;
	}
