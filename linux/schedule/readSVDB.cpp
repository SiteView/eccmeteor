...
map<string, OBJECT> readSVDB::svMonitors;
map<string, OBJECT> readSVDB::svEntities;
...
{   //如果配置信息有变更，需要读取新的，请调用此函数（将把以下 map 中的内容清空，再次需要时，自动重读新的）
	...
	for(map<string,OBJECT>::iterator it= svMonitors.begin(); it!=svMonitors.end(); ++it)
		CloseMonitor( it->second );
	svMonitors.clear();

	for(map<string,OBJECT>::iterator it= svEntities.begin(); it!=svEntities.end(); ++it)
...
{
	map<string, OBJECT>::iterator it= svMonitors.find(id);
	if( it != svMonitors.end() )
		return it->second;
	...
	svMonitors.insert(make_pair(id, obj));
	return obj;
}
...
{
	map<string, OBJECT>::iterator it= svEntities.find(id);
	if( it != svEntities.end() )
		return it->second;
	...
	svEntities.insert(make_pair(id, obj));
	return obj;
}
...
int readSVDB::GetSubsequentList( CSubsequentItemList &lstItem )
{
	...
	if ( ! GetIniFileSections(filsec,"subsequent.ini",svdbaddr) )
		throw MSException("Can't load data from svdb.\n\nPlease check svdb available! \n");
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool readSVDB::GetSMCommon( Monitors *pM, string strID )
{
	bool bret=true;
