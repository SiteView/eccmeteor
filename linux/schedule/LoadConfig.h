...
public:
	...
	BOOL CreateSingleEntity(Entitys *pE,string strEntityID);		//创建一个 entity 传回5个参数
	BOOL CreateSingleMonitor(Monitors *pM,string strMonitorID);		//创建一个 monitor 传回 para, return, stacon 等
...
private:
	...
	void LoadSingleEntity(Entitys*pEntity,string strEntityID);
	void LoadEntitys(CEntityList &EntityList);
	BOOL GetStateCondition(Monitors *pM,string strMonitorID);
	BOOL GetReturnList(Monitors*pM,int MonitorType);
	...
	BOOL GetParamList(Monitors *pM,string strMonitorID, string strParentID);
	BOOL GetMonitorPro(Monitors *pM, string MonitorID);
	BOOL m_isLoad;
